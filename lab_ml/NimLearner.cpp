...
NimLearner::NimLearner(unsigned startingTokens) : g_(true, true) {
    ...
    for (unsigned int i = startingTokens - 1; i > 0; i--) {
      ...
      g_.insertEdge("p1-" + to_string(i+1), "p2-" + to_string(i-1));
      g_.setEdgeWeight("p1-" + to_string(i+1), "p2-" + to_string(i-1), 0);

      g_.insertEdge("p2-" + to_string(i+1), "p1-" + to_string(i-1));
      g_.setEdgeWeight("p2-" + to_string(i+1), "p1-" + to_string(i-1), 0);
    }
...
std::vector<Edge> NimLearner::playRandomGame() const {
  ...
  while(next != "p1-0" && next != "p2-0") {
    std::vector<Vertex> adjacents = g_.getAdjacent(cur);
    if (adjacents.size() != 1) {
      next = adjacents.at(rand() % 2);
    } else {
      next = adjacents.at(0);
    }
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
  /* Your code goes here! */
  if (path.at(path.size()-1).dest == "p2-0") {
    for (size_t i = 0; i < path.size(); i++) {
      if (i % 2 == 0) {
        g_.setEdgeWeight(path.at(i).source, path.at(i).dest,
        g_.getEdgeWeight(path.at(i).source, path.at(i).dest) + 1);
      } else {
        g_.setEdgeWeight(path.at(i).source, path.at(i).dest,
        g_.getEdgeWeight(path.at(i).source, path.at(i).dest) - 1);
      }
