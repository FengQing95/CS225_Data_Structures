...
size_t heap<T, Compare>::maxPriorityChild(size_t currentIdx) const
{
    ...
    if (_elems.size() - 1 < rightChild(currentIdx)) {return leftChild(currentIdx);}
    size_t leftInd = leftChild(currentIdx);
    size_t rightInd = rightChild(currentIdx);
    return higherPriority(_elems[leftInd], _elems[rightInd]) ? leftInd : rightInd;
}
...
void heap<T, Compare>::updateElem(const size_t & idx, const T& elem)
{
    ...
    heapifyUp(idx);
    heapifyDown(idx);
    //for (size_t i = 1; i < _elems.size(); i++) {heapifyDown(i);}
}
