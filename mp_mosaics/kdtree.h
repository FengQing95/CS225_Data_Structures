...
class KDTree
{
  ...
  private:
     ...
     */
     void pick(int left, int right, int k, int d);
     vector<Point<Dim>> pointVec;
     KDTreeNode * buildTree(int left, int right, int d);
     void exchange(int i, int j);
     void delete_helper(KDTreeNode * root);
     Point<Dim> findNearestNeighbor_helper(KDTreeNode * root, const Point<Dim> target, Point<Dim> cur, int d) const;
     int divide(int left, int right, int pivot, int d);
};
