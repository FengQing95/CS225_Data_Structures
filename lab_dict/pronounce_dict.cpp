...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (dict.find(myWord1) == dict.end() || dict.find(myWord2) == dict.end()) {
      return false;
    }

    std::vector<std::string> pron1 = dict.find(myWord1)->second;
    std::vector<std::string> pron2 = dict.find(myWord2)->second;
    if (pron1.size() != pron2.size()) {
      return false;
    }
