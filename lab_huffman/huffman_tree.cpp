                            ...
                            queue<TreeNode*>& mergeQueue)
{
  if(singleQueue.empty() && mergeQueue.empty()){return NULL;}
    ...
    TreeNode * res = mergeQueue.front();
    mergeQueue.pop();
    return res;
  }
  else if(mergeQueue.empty()){
    TreeNode * res = singleQueue.front();
    singleQueue.pop();
    return res;
  }
  else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for(auto itr = frequencies.begin(); itr != frequencies.end(); ++itr){
       TreeNode * tem = new TreeNode(*itr);
       singleQueue.push(tem);
     }

     // remove, create, palce back
     while(true){
       if(singleQueue.size() + mergeQueue.size() == 1){break;}

       std::vector<TreeNode *> v;
       for(int i = 0; i < 2; i++){v.push_back(removeSmallest(singleQueue, mergeQueue));}

       TreeNode * newNode = new TreeNode(v[0]->freq.getFrequency() + v[1]->freq.getFrequency());
       newNode->left = v[0];
       newNode->right = v[1];
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  ...
  if(bfile.hasBits()){
    if(bfile.getNextBit()){
      res = new TreeNode(Frequency(bfile.getNextByte(), 0));
    }
    else{
      res = new TreeNode(0);
